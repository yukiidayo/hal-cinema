...
export default function ConfirmPage() {
  ...
  const { schedInfo, totalPrice, customer, seats } = useConfirm()
  ...
  return (
    <div className="py-6 max-w-xl mx-auto">
      ...
      <section className="mb-6 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
        ...
        <p className="mt-2 text-sm text-gray-700"><span className="font-bold text-gray-400">メール</span> {customer?.email}</p>
      </section>

      <div className="flex flex-col gap-3">
        <Button size="lg" className="h-14 text-base font-black shadow-lg shadow-red-600/20"
          onClick={() => navigate("/reservations/payment")}>お支払いへ進む</Button>
        <Button size="md" variant="ghost" onClick={() => navigate(-1)}>戻る</Button>
      </div>
    </div>
  )
}
